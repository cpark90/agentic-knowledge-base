...
채널 규약 원본: `docs/feedback/README.md` — 세션 시작 시 읽는다.
...
- 유저의 **구체화·제안**: 유저 lane 항목으로 정리한다 — 관련 지식(청크 IRI·ODD
  조건·결정)을 찾아 `targets`에 링크하고, 반영 계획이 승인 가능할 만큼 구체적인지
  확인한다. 모호하면 유저에게 되묻는다 (되묻기는 hci의 일이다).
