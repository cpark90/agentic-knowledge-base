...
않고 지식(청크·ODD·온톨로지)으로 승격되며, 그 승격이 이 채널의 목적이다.
...
|---|---|---|
| **유저** | 전체 | 전체 |
| **hci** | `docs/feedback/**` 전체 — 이것이 hci의 유일한 쓰기 범위다 | 저장소 전체 |
| **다른 에이전트** | `agents/` lane의 **자기 항목만** (유저에게 전달할 내용) | 저장소 전체 (읽기 전용) |

- hci는 지식 산출물(ontology/·odd/·kg/·chunks/·tools/ 등)을 편집하지 않는다.
...
- **hci → 유저**: 결정 요청·에이전트 lane에서 올라온 피드백 중계를 항목으로 남긴다.
  frontmatter에 `status: open`을 **반드시 포함**한다 — 유저는 필드를 새로 적지 않고
