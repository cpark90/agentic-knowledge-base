...
**아직 절차가 없다.** 코어이 무엇을 남겨 두었는지만 정해져 있다 — 프로파일은 코어 클래스의
하위 클래스와 shape만 추가하고 코어을 수정하지 않는다
...
4. 검토했으나 밖에 두는 것을 **명시 제외**에 기록한다 — 서식은
   ...
   것이 이 절의 존재 이유다.
5. 현재 **실제 조건이 ODD 안에 있는지 대조**한다. 첫 모니터링에서 이탈이 나오면 틀린 쪽은
   현실이 아니라 ODD다.
...
`functional → abstract → logical → concrete → executable`. 각 정제은 **근거와 `refines`
...
**모든 뷰은 질의의 결과이며 저장하지 않는다.** 저장된 뷰는 원본과 어긋나는 순간부터
...
마지막 전이가 특별하다 — 어휘가 자라면 다음 프로젝트는 자란 어휘로 시작한다. **일반화은
...
v1의 "검증은 응용"(d-0130)은 v3부터 대체됐다 — 검증·확인는 **두 번째 지식 베이스**이며 절차는
§14 V&V 층에 있다 ([`p12-dev-vv-kb-exchange`](../kb/dev/decision/p12-dev-vv-kb-exchange/conclusion.md)).
코어이 제공하는 것은 시험 대상·경계(ODD)·결과 축적(실행 기록)·실패 분류(defect)의 단위다.
