...
**CQ-04 ✅ 어느 항목도 구성체에 속하지 않는가 (고아 후보).**
`?c a agt:Chunk FILTER NOT EXISTS { ?x agt:hasDirectPart ?c }`.
*실측: 0건. 단 구성체는 통합이 필요한 것만 만들므로(유저 결정) 이 값이 0이어야 하는 것은
아니다 — 링크가 생기면 "구성체에도 링크에도 걸리지 않은 것"으로 정의가 바뀐다.*

**CQ-05 ✅ 구성체의 직접 부분이 9개를 넘는가 / 부분의 plane·level이 전체와 다른가.**
...
실행 기록(`agt:Run`)과 `defect` 어휘가 없다. 상승 트리거의 입력이다.
...
그리고 **`functional`·`abstract`·`logical`·`executable`** — 이 넷은 진짜 미사용이며 사다리가
...
노트 2.7절의 경쟁 질문 20개와 이 문서의 CQ-01~32는 **번호도 내용도 다르다** —
...
| **CQ9** | **지난 프로젝트에서 같은 조건의 배제 선택** | 없음 | 프로젝트 간 상승 — profile/ 이후 |
...
| CQ19 | executable까지 닿지 않은 요구 (하강 완주) | CQ-13 확장 | **v3의 완료 판정 축** |
| CQ20 | 요구로 거슬러 오르지 못하는 산출물 (상향 귀속) | CQ-13 확장 | 〃 |

**등재 필요 4건**(CQ9·11·12·16)은 어휘·데이터가 생기는 시점에 이 문서의 해당
절로 들어온다 — 새 개념을 추가할 때 어느 경쟁 질문에 기여하는지 적는 규칙
...
**추이**: 2026-09-04 `related/trace` 추가로 ❌ 13 → 6. 2026-09-07 트러스트 티어와 인용 추출로
...
하네스가 읽기·쓰기 집합을 기록해야 한다. 그다음이 검증·상승 어휘(CQ-12·26·27).
