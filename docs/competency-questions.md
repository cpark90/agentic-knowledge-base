...
3. **질문을 답할 수 없게 만드는 변경은 검토 대상이다.** 아래 상태가 ✅에서 ❌로 내려가는
   변경은 리뷰에 걸린다.
4. 답은 질의이므로 **저장하지 않는다**. 아래 실측값은 2026-09-04의 질의 결과다. `query`
...
읽기의 기본 진입점이다. `?c a ?planeClass ; rdfs:label ?l`.
*실측: `decision`×`concrete` 153건. 다른 칸은 비어 있다.*

**CQ-02 ✅ 이 항목은 어느 칸에 있는가 (plane × level).**
`?c a ?plane ; agt:hasLevel ?level`. 격자 배치와 통계의 기초다.
...
아직 어휘에 없다. d-0087이 요구하는 다섯 유형이 미구현이다.*

**CQ-11 ⚠ 이 스코프는 ODD의 어느 부분집합인가 — 이 역할이 볼 수 있는 조건은 무엇인가.**
`?s agt:subsetOf ?odd ; agt:includesCondition ?c`.
*실측(2026-09-13): 스코프 4개 전부 `subsetOf` 보유, 참조 조건 6/7. 다만 "스코프 밖 의존"은 가정이 비어 있어
답이 무의미하다. 스코프 밖 의존은 어떤 항목이 스코프 밖 조건에 의존하는가를 묻는다.*
...
*실측: 인용 링크 27개로 **첫 답이 나온다**. 예를 들어 d-0002가 바뀌면 그것을 인용하는 항목이
...
*실측(2026-09-13): 역할 4 — orchestrator·developer·vnv·hci(inspection은 hci로 이관). read plane 0인 역할은 없다.
write는 orchestrator=decision · developer=artifact · vnv=annotation로 겹치지 않는다.*
...
*실측: 정의된 `agt:` 용어 59개 중 데이터에서 쓰인 것 44개, 미사용 15개다. 미사용은
...
대응은 아래와 같다. **굵은 행이 이 문서에 없던 노트 질문**이다.
...
**추이**는 다음과 같다. 2026-09-04 `related/trace` 추가로 ❌ 13 → 6이 되었다. 2026-09-07 신뢰 등급과
인용 추출로 ✅ 13 → 16이 되었다. 이때 질문 2개를 추가했다. 어휘가 막는 것은 6건뿐이고 나머지
10건은 **데이터**가 막는다. 어휘 공백과 데이터 공백을 가르는 것이 이 표의 목적이다.
