...
지어낸 용어를 쓰지 않는다는 원칙(노트 0.0절)의 뒷면이다 — 표준에서 가져왔으면
...
| 정보 내용 개체 — `agt:Chunk`의 상위 클래스 | IAO |
...
| 가져온 구조 | 출처 |
|---|---|
| 청크 — 최소 정보 단위, 7±2, 라벨 필수, 한 주제 | 인지과학(Miller), 구조적 글쓰기(Horn) |
...
체계는 **도메인 중립 골격**과 **도메인 프로파일**로 나뉜다. 아래는 골격을 개발
작업에 특수화한 결정들이며, 다른 프로파일(문서 작성·운영·분석)의 템플릿이다.

| 골격 항목 | 개발 프로파일의 결정 |
|---|---|
| `decision` 청크 | 설계 결정. 결론·근거·배제된 대안의 구성체 (ADR 형식) |
| `schema` 청크 | 프로토콜 메시지·필드. 스키마 언어로 기술 |
| `contract` 청크 | 인터페이스 시그니처. 언어 네이티브 선언 |
| `artifact` 청크 | **함수.** 42줄 초과 함수는 분할 |
| `annotation` 청크 | 코드 주석, 리뷰 코멘트. 코드 밖에 저장 |
| 앵커 해석 | 심볼 ID → 파일 내 범위 |
| 판정 도구 | 컴파일러, 타입 체커, 스키마 검사기, 테스트, 린터 |
| 투영 | tangle(코드 파일), weave(문서) |
| 결함 하위 유형 | ODC 8유형 |
| 경계 조건 셋째 수준 | 언어·런타임, 빌드 체계, 저장소 구조, 의존성 버전 |
| 에이전트 역할 | orchestrator, developer, design, V&V, inspection, research, claim, audit |
| 검증 환경 | 단위 테스트 → 계약 검사 → mock 통합 → 실제 인프라 → 스테이징 → 운영 |

**이 저장소는 이 프로파일의 부분집합을 쓴다.** 현재 `decision` 청크만 존재하고
역할은 5개(orchestrator·developer·vnv·inspection·hci)다 — 프로파일은 가능한
전체를 정의하고, 프로젝트는 필요한 만큼 쓴다. 실제 배치는
[`artifact-placement.md`](artifact-placement.md).

## 3. 관련 산출물과 사례

- **`CODESTYLE.md` / `CLAUDE.md`** — 구현 저장소의 코드 스타일과 하네스 규약.
  이 저장소의 대응물은 `STYLEGUIDE.md`와 `AGENTS.md`.
- **하네스 지식** — 이 체계의 사다리로 두 저장소에 재배치됨:
  `../harness-functional`(ODD + functional: 어휘 TBox·shapes·도구),
  `../harness-concrete`(logical + concrete: union root·부품 라이브러리·조립 명세).
  두 저장소의 일반 방법론은 청크 d-0013~d-0020으로 승격되어 있다.
