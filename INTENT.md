...
| 문서 | 자리 |
|---|---|
| [`docs/agent-knowledge-system-notes.md`](docs/agent-knowledge-system-notes.md) (v5, 3,470줄) | 요구의 **근거와 결정의 원천**. Part I·산출물 정의가 요구를 낳고, Parts 0~XIII의 `[확정]`이 결정으로 재도출된다 (v3 기준 145건 + v4·v5 델타 37건 = 182건, 2026-09-10) |
...
`refines`한다. 하강 완주율(CQ19)·상향 귀속률(CQ20)의 분자가 여기서 시작된다.
...
| **검증자** | 만든 쪽과 판정하는 쪽이 구조적으로 분리된 채 모든 높이에 가로대가 걸린다 |

### 업체 — 분야 지식의 축적과 재사용

- [`r-001`](kb/dev/requirement/) 분야 지식을 어휘로 축적한다 — `id:chunk/27699a04…`
- [`r-002`](kb/dev/requirement/) 프로젝트를 넘는 것은 어휘·공리·골격·교훈이다 — `id:chunk/d8e8aa97…`
...
- [`r-010`](kb/dev/requirement/) 결정은 봉사하는 관심사를 명시한다 — `id:chunk/57a2dca5…`
- [`r-011`](kb/dev/requirement/) 근거 없는 배정은 불가능해야 한다 — `id:chunk/f715c53f…`
- [`r-012`](kb/dev/requirement/) 변경은 링크를 재판정 대상으로 만든다 — `id:chunk/33419d0a…`

### 에이전트 — 좁은 컨텍스트에서의 판단 — 관측·통제·상승
...
- [`r-023`](kb/dev/requirement/) 가로대 없이 다음 높이로 내려가지 않는다 — `id:chunk/71d2b786…`
