---
id: https://agentic-knowledge-base.dev/id/chunk/57a2dca5-8628-4255-a964-9928d2fd14a4
type: requirement
level: functional
title_ko: 결정은 봉사하는 관심사를 명시한다
title: A decision names the concern it serves
status: stable
sources: [https://agentic-knowledge-base.dev/id/doc-system-notes, https://agentic-knowledge-base.dev/id/doc-structure]
generated: {by: claude/fable-5, at: 2026-09-10T16:00:00+09:00}
---
**요구** — 결정이 저작되면, 체계는 그 결정이 어느 요구의 어느 관심사에 봉사하는지 명시하여야 한다.

- **이해관계자**: 프로젝트 · **관심사**: 하강 완주
- **출처**: 구조도 대상 지식(결정) / 노트 6.8절

요구와 관심사를 명시하지 않은 결정은 refines의 도착점을 말할 수 없다.
