---
id: https://agentic-knowledge-base.dev/id/chunk/f715c53f-c9bd-49cc-b580-6e2d2343cd5c
type: requirement
level: functional
title_ko: 근거 없는 배정은 불가능해야 한다
title: Groundless assignment must be impossible
status: stable
sources: [https://agentic-knowledge-base.dev/id/doc-system-notes, https://agentic-knowledge-base.dev/id/doc-structure]
generated: {by: claude/fable-5, at: 2026-09-10T16:00:00+09:00}
---
**요구** — 후보가 여럿 남아 있는 동안, 체계는 근거 없이 하나를 배정하는 것을 자료구조 수준에서 불가능하게 하여야 한다.

- **이해관계자**: 프로젝트 · **관심사**: 미확정의 보존
- **출처**: 노트 1.2절·Part IX

'추측하지 마라'는 지시는 성립하지 않는다 — 후보가 여럿인 상태를 정상으로 두는 구조만이 막는다.
