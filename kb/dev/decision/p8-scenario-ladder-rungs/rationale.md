...
type: decision
level: logical
title_ko: 가로대를 게이트 조건으로 두면 검증을 나중으로 미룰 수 없다
...
---
**근거** (노트 8.3절) — 가로대를 게이트의 통과 조건으로 두면 "검증은 나중에"가 구조적으로 불가능해진다. 개발 사다리가 혼자 내려가면 산출물이 나온 뒤에야 무엇으로 판정할지 정하게 되고, 그때 기준은 산출물에 맞춰진다.
...
- `refines` 연쇄가 끊기면 V&V KB의 하강 완주율에 잡힌다 — 두 KB에 각각 완주율이 있다 (7.7절).
