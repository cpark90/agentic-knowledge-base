...
type: decision
level: concrete
title_ko: 검증 청크는 판정 가능한 가장 낮은 단계에 배정한다
...
composite: {id: https://agentic-knowledge-base.dev/id/composite/12df135b-c37d-48d8-b768-8279da163438, title_ko: 검증 청크의 환경 배정, title: Assigning verification chunks to environments}
---
**결론** — **각 검증 청크는 그것을 판정할 수 있는 가장 낮은 단계에 배정한다.** 배정은 태그(0.5절)에 `env:` 범주로 기록한다.
...
**배정 근거는 검증 청크의 결함 요인 태그다** — 실행 요인은 1~2단계에서 잡히고, 상호작용 요인은 3단계 이상이 필요하며, 인지 요인은 에이전트 검증 3단계가 필요하다. 요인 분류가 환경 배정을 결정한다.
