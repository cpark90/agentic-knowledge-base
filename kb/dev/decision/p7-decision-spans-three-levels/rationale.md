...
---
**근거** (노트 7.2절, 6.2절, 9.10절) — abstract는 변수의 선언이 온톨로지 어휘만 쓰는가로, logical은 후보에 범위·제약·배제 근거가 있는가로, concrete는 표본 근거와 배제 근거가 있는가로 판정된다 (6.8절 게이트). 판정이 다른 것을 한 청크에 두면 한 수준의 실패가 다른 수준을 `draft`로 끌어내린다. `refines`로 잇기 때문에 하강 완주율(7.1절)이 결정 안에서도 계산된다.
