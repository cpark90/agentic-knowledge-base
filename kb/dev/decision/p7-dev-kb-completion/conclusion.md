...
type: decision
level: concrete
title_ko: 개발 KB의 완결 조건은 하강 완주와 상향 귀속 100%이고 완료는 V&V 상태를 읽어야 선언된다
...
---
**결론** — 개발 KB의 완결 조건은 하나다: **모든 요구가 executable까지 `refines` 연쇄로 닿고, 모든 산출물이 요구로 거슬러 오른다** (CQ19·CQ20). 하강 완주율·상향 귀속률이 100%가 아니면 개발 KB는 열려 있다 (노트 7.1절).
