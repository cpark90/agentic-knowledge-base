...
type: decision
level: concrete
title_ko: 논평과 산출물을 분리하는 도구가 필요하다
...
composite: {id: https://agentic-knowledge-base.dev/id/composite/67157f22-93ba-4abe-a303-d058de25ef7f, title_ko: 논평과 산출물의 분리, title: Separating annotation from artifact}
---
**결론** — **논평과 산출물을 분리하는 도구가 필요하다.** 논평이 산출물에
인라인으로 박혀 있는 한(코드 주석, 문서의 여백 메모) 두 plane은 항상 함께
적재된다.

`annotation` 청크는 대상 청크 IRI를 갖는 **standoff 앵커**로 붙고(5.1절 청크
ID의 해석), 산출물 파일과 설명 문서는 각각 다른 투영 질의의 결과다(4.6절).
