...
composite: {id: https://agentic-knowledge-base.dev/id/composite/85fcc7f3-446f-46d6-b386-693544fefe6e, title_ko: 구성체 — 표준 부분-전체 관계, title: Composite as standard parthood}
---
**결론** — **구성체(`agt:Composite`)** 는 청크 또는 다른 구성체를 부분으로 갖는
개체다. **assertion 그래프가 없다** — 라벨과 순서 있는 부분 목록이 전부다.

- 부분-전체는 **상위 온톨로지의 `part-of`** 를 쓴다.
- **순서는 표준 순서 컬렉션 어휘로 쓴다.** 순서가 필요한 구성체(케이스 열,
  절차, 논증)는 Collections Ontology의 `co:List`를 겸하며 각 부분이
  `co:index`를 갖는다.
- 순서가 없는 구성체(스키마의 필드 집합)는 `part-of`만 쓴다.
