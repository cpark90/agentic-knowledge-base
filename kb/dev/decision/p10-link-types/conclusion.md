...
---
**결론** — 링크 타입은 확립된 의미를 가진 것만 쓰고, **수직**(계층를 따라)과 **수평**(plane을 가로질러)으로 나눈다.

**수직** — `refines` (level n+1 → level n). 6.2절 정제의 모든 전이가 이 링크를 남기며, **계층는 링크로 구현된다.** concrete에서 `refines`를 거슬러 functional까지 닿지 않으면 단계 건너뛰기다.
...
특수한 셋 — `refines`만 계층를 따라, `assumes`만 plane을 가로질러 무효화를 전파, `prov:wasRevisionOf`만 시간을 가로질러 잇는다.
