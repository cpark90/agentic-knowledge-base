...
답하는 질문과 근거 — 무엇을 요구받았는가(`requirement`) / 어떻게 정했고 무엇을 배제했는가(`decision` 구성체) / 구성요소끼리 무엇을 약속했는가(`contract`) / 데이터가 어떤 형태로 오가는가(`schema`) / 실제로 무엇이 만들어졌는가(`artifact` 앵커) / 이 전부가 어떤 조건 위에 서 있는가(ODD + `assumes`) (노트 7.1절).
