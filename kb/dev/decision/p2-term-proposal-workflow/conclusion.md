...
---
**결론** — 상승(6.3절)이 온톨로지에 닿을 때의 절차. OBO 진영에서 운영되는 흐름을 그대로 쓴다.

1. 에이전트가 관측 청크에서 개념 후보를 뽑아 **ROBOT template 행**으로 쓴다 — ID · 라벨 ko/en · 정의(속 + 종차) · 상위 클래스 · 기여하는 경쟁 질문
2. template → OWL 변환. **2.5절 3계층 검사**
3. 통과한 것만 **유저 승인 큐**로
4. 승인되면 **코어가 아닌 확장 모듈**에 병합하고, `prov:wasDerivedFrom`으로 관측 청크에 연결

**template 행이 곧 청크다** — 42줄 안, 라벨 필수, 4분 구조. 온톨로지 확장 제안이 다른 지식과 같은 규율을 따른다.
