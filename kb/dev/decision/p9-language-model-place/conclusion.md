...
type: decision
level: concrete
title_ko: 언어모델은 후보와 선호 순서를 제안할 뿐이고 선호의 기본은 증거 기록의 종류 서열이다
...
---
**결론** — 언어모델의 확률적 추론은 두 곳에만 쓴다 — **후보를 제안**하고(증거 기록에 `kind: proposal`, 극성 +, 확정 근거 아님), 후보가 여럿일 때 **선호 순서를 제안**한다(9.9절 부분순서, 이유 문자열 필수). 확정·기각·상태 전이는 증거 기록 규칙이 한다. 이것이 2.5절 "에이전트는 센서"의 링크판이다.

선호는 증거 기록에서 파생될 수 있다. 지지 증거의 종류 서열(구축 > 실행 > 동시 편집 > 공동 커버 > 임베딩 > 세션)로 후보를 정렬한 것이 기본 선호이고, 언어모델의 제안은 이를 덮지 못하고 동률만 깬다 (노트 9.11절). 이 저장소: 증거 종류 `agt:proposal`, `agt:counterfactualTest` 폐기 (Q10).
