...
---
**결론** — 세 층 (노트 부록 E.1): **OKF v0.2** — 청크의 물리 형식. 파일 하나 = 청크 하나, 프런트매터 = head, 본문 = assertion / **온톨로지(Turtle + LinkML)** — 어휘·제약. `type`은 plane 클래스로 제한, 관계는 TIM 링크 타입으로 제한 / **Bazel** — 확정 링크 = deps, 게이트 = test, 뷰 = build output, 재판정 = 증분 재실행.
