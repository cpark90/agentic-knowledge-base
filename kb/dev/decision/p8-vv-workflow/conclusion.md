...
1. 검증 목표 파생 — `requirement`(vv), 요구마다 최소 하나, `derives-from` · 2. 시나리오 형식화 — `decision`(vv) abstract, 변수가 ODD 속성 · 3. 논리 시나리오 + 기준 — `decision`(vv) logical `keep(범위)` / `contract`(vv) logical 판정식 · 4. 케이스 생성(자동) — concrete `keep(고정값)`, 표본 근거 `cover()` · 5. verifier 작성 — `artifact`(vv), 기준 바인딩 · 6. 환경 배정 — `env:` 태그, 재현성 · 7. 실행(하네스) — `memory`(vv), seed·리비전 · 8. 판정 — verifier + 판정자, `annotation`(vv) 결함 분류, 3지표 · 9. 보고 — 투영, 저장하지 않음 · 10. 되먹임 — 지도 청크 → 상승 후보.
