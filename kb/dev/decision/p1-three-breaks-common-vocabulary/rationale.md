---
id: https://agentic-knowledge-base.dev/id/chunk/dfdd6089-bd08-4539-8351-7f342c46661b
type: decision
level: logical
title_ko: 각 단절이 생기는 이유와 어휘가 선행하는 이유
title: Why each break occurs and why vocabulary comes first
status: stable
sources: [https://agentic-knowledge-base.dev/id/doc-system-notes]
generated: {by: claude/fable-5, at: 2026-09-10T21:30:00+09:00}
part_of: https://agentic-knowledge-base.dev/id/composite/e84e779c-80d6-41e0-bee2-7b75fe0ac090
---
**근거** (노트 1.3절) — 하강이 끊어지는 이유: 에이전트는 의도에서 곧바로 산출물로
가며 중간 기록을 남길 이유가 없다. 상승이 끊어지는 이유: 세션이 끝나면 경험이
사라져 같은 실수를 다음 세션이 반복한다. 갱신이 끊어지는 이유: 무엇이 무효가
됐는지 알 방법이 없어 전수조사밖에 없다.

의도 문서·산출물·시행착오 기록이 각자 다른 언어로 쓰여 있으면 서로 참조할 수
없고, 참조할 수 없으면 내려갈 수도, 올라갈 수도, 무엇이 깨졌는지 셀 수도 없다.
그래서 세 단절을 각각 고치는 세 장치가 아니라, 공통 원인인 어휘를 먼저 놓고 그
위에 하강·상승·무효화를 세운다.
