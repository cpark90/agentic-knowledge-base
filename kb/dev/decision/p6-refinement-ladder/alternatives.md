...
배제 이유 — 그 정의에서는 `requirement` plane이 따로 없어 functional이 `decision`에 얹히고, 산출물의 executable과 결정의 executable이 같은 뜻이 되어 계층이 **무엇에서 무엇으로 가는 사슬인지** 말하지 못한다. 정제 수준으로 정의하면 양 끝이 plane 전용이 되고, 격자가 성기다는 결과(6.4절)와 검증 KB가 executable을 나눠 갖는다는 결과(7.2절)가 따라 나온다.
