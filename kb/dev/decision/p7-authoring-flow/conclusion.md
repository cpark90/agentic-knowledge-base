...
type: decision
level: concrete
title_ko: 요구 하나는 여덟 단계로 산출물이 되고 요구마다 독립적으로 하강한다
...
| 2. 형식화 | design | `decision` abstract — 변수 선언, `serves` | 봉사 없는 설계 거부 | — |
...
| 7. 구현 | developer | `artifact` executable — 함수 | 컴파일·린터 + `satisfies`·`refines` | verifier 존재 + 바인딩 |
| 8. 리뷰 | V&V 또는 다른 developer | `annotation` | 해소 | 판정 논평 |

순서는 요구 하나에 대한 것이지 프로젝트 전체가 폭포식이라는 뜻이 아니다. 요구마다 독립적으로 하강하고, 요구 간 순서는 `depends-on` 링크와 ODD 동적 갈래의 시간 제약이 정한다.
