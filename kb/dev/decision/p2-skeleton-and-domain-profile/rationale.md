...
type: decision
level: logical
title_ko: 골격이 도메인에 오염되면 다음 도메인이 골격을 못 쓴다
...
---
**근거** (노트 2.11절) — 프로젝트를 넘어 재사용되는 것은 골격이다. 골격에 개발 도메인의 개념(심볼 앵커, ODC 결함 유형)이 섞이면 문서 작성이나 운영 프로파일이 그것을 상속받아 무의미한 제약을 진다.

프로파일을 온톨로지 모듈로 두면 2.3절 "확장은 새 모듈 추가로만"이 그대로 적용되고, 골격 수정 여부를 기계로 검사할 수 있다 — 프로파일 모듈이 골격 클래스에 대한 공리를 추가하면 3계층에서 실패한다.
