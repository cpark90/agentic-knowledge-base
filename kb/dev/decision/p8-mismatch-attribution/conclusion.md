...
---
**결론** — 검증기가 실패하면 세 가지 중 무엇이 틀렸는지 판단해야 한다 — **산출물**(구현이 요구를 못 지킴), **지식**(요구·기준·가정이 현실을 못 담음), **둘 다**. 이 판단은 자동이 아니며 하나의 **결정**이다. V&V KB `decision` plane에 **지침(guidance)**로 기록한다 — 결론(귀속과 조치), 근거(진단), 대안(배제된 귀속) (노트 8.4절).
