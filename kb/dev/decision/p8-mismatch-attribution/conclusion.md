...
type: decision
level: concrete
title_ko: verifier 실패의 귀속은 결정이며 V&V decision의 지도 청크로 남는다
title: Attributing a verifier failure is a decision, recorded as a guidance chunk in V&V decision
status: stable
...
---
**결론** — verifier가 실패하면 세 가지 중 무엇이 틀렸는지 판단해야 한다 — **산출물**(구현이 요구를 못 지킴), **지식**(요구·기준·가정이 현실을 못 담음), **둘 다**. 이 판단은 자동이 아니며 하나의 **결정**이다. V&V KB `decision` plane에 **지도(guidance) 청크**로 기록한다 — 결론(귀속과 조치), 근거(진단), 대안(배제된 귀속) (노트 8.4절).

| 귀속 | 조치 | 되먹임 목적지 |
|---|---|---|
| 산출물 | 구현 수정 요청 | 개발 KB `artifact` — 결정은 그대로 |
| 지식 — 요구 과도 | 요구 완화 | 개발 KB `requirement` 상승 |
| 지식 — 제약 부족 | 계약·범위 강화 | 개발 KB `contract`·`decision` logical 상승 |
