...
composite: {id: https://agentic-knowledge-base.dev/id/composite/33d988f2-27b2-44d7-98f2-110215bfe237, title_ko: 온톨로지 컴파일러 — 검사 3계층, title: The ontology compiler and its three check tiers}
---
...
**안티패턴 계층이 이 체계의 게이트 대부분을 흡수한다.** "근거 없는 배정"(Part IX), "ODD에 없는 조건을 참조하는 가정"(3.3절), "`sources`가 빈 청크"(4.3절), "`verifies`에 기준이 없음"(7.11절), "거주표 밖의 plane·level 조합"(6.4절)은 전부 "이런 트리플이 존재하면 실패"라는 SPARQL 한 줄이다. shape로 쓰기 어색한 제약은 verify 질의로 쓴다.
