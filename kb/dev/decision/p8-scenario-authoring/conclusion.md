...
type: decision
level: concrete
title_ko: 시나리오는 부류에서 시작하는 V&V decision 구성체이고 concrete는 사람이 쓰지 않는다
...
---
**결론** — 시나리오는 `decision`(vv) 구성체다. 결론·근거·대안이 각각 **자극**(actor·action·순서·`keep()`)·**요인**(노출하려는 결함 요인, 봉사하는 검증 목표)·**배제 자극**(다루지 않기로 한 자극과 이유 — ODD 밖, 다른 시나리오가 덮음)이다 (노트 8.22절).
