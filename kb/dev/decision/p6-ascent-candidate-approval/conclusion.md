...
---
**결론** — 일반화은 **자동화하지 않는다.** 후보를 제시하고 유저 확인을 받으며, 온톨로지 갱신은 2.5절 용어 제안 워크플로(template 행 → 3계층 검사 → 유저 승인 → 확장 모듈 병합)를 따른다.

후보의 우선순위는 **실행 가능 피드백**이다. 판정 도구의 결과(테스트·검사)가 붙은 관측 청크가 산문 관찰보다 먼저 승격된다. 피드백 자체의 신뢰성이 낮으면(7.13절 시뮬레이션 신뢰도) 일반화이 오염되므로, **신뢰도가 검증되지 않은 환경의 관측은 승격을 보류한다.**
