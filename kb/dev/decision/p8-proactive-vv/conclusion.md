...
| 트리거 | 동작 | 산출 |
|---|---|---|
| ODD 대조 주기 (`odd_check`) | 실제 조건이 경계 근처면 그 속성을 자극하는 시나리오를 우선 실행 | 이탈 전 경고, 관측 |
| 실행 이력의 공백 | `cover()` 미달 구간, 오래 실행되지 않은 verifier를 골라 실행 | 커버리지 갱신 |
| 외부 지식 변화 | 표준·규제·의존성 릴리스 노트에서 새 조건 후보 추출 | ODD 확장 제안, 새 검증 목표 후보 |
| 확정 링크의 증거 노화 | 마지막 지지 증거가 오래된 `satisfies`·`verifies`를 재실행 | 장부 갱신 (9.11절) |

선제적 V&V의 산출은 전부 **후보**다. 새 검증 목표는 개발 KB 요구에서 파생되지 않았으므로 `origin:observed` 태그를 달고, 8.4절 귀속 판단을 거쳐 요구 상승 후보가 된다.
