...
composite: {id: https://agentic-knowledge-base.dev/id/composite/b4546452-61a1-4d61-b6c3-8af8a25e0f44, title_ko: 모든 지식은 구성체다, title: All knowledge is composite}
---
...
| 지식 항목 | 청크 클래스 | 구성체 |
|---|---|---|
| 결정 | `DecisionChunk` — 결론·근거·배제된 대안 | **세 청크의 구성체** |
| 요구 | `RequirementChunk` — 요구 문장 (EARS) | 관심사별 구성체 |
| 합격 기준 | `DecisionChunk`·`ContractChunk` logical — 판정식 | 기준 집합 |
| 시나리오 (V&V) | `DecisionChunk` — 자극·요인·배제 자극 | 세 청크의 구성체 |
| verifier (V&V) | `ArtifactChunk` executable | 케이스 열의 `co:List` |
| 시그니처 | `ContractChunk` | 대부분 청크 하나 |
| 스키마 | `SchemaChunk` — 메시지·필드 | 순서 없는 구성체 |
| 모듈 | `ArtifactChunk` — **함수** | 순서 있는 구성체 |
| 리뷰 스레드 | `AnnotationChunk` — 코멘트 | 순서 있는 구성체 |
| ODD | 속성 청크 | 절 구성체의 구성체 |
| 실행 기록 | `MemoryChunk` — 관측, append-only | 시간순 구성체 |
