...
type: decision
level: concrete
title_ko: 보고서는 저장하지 않고 뷰하며 표준 보고 다섯과 지표 일곱을 읽는다
...
---
**결론** — 보고서는 저장하지 않고 뷰로 낸다 (4.6절). 표준 보고 다섯 (노트 8.24절): **검증 상태**(요구별 목표·기준·검증기 수, 마지막 실행 결과, `verifies` 상태 — level×level 매트릭스 + 실행 기록) · **커버리지**(전방 추적 커버리지·후방 추적 커버리지·logical 공간 커버·경계값 커버) · **결함**(요인·트리거·발견 단계 분포 — `defect-rules`) · **가정 건전성**(`invalidated`·`unverified` 가정과 의존 청크 수) · **독립성**(개발 역할이 V&V KB에 쓴 흔적, 0이어야 함).

보고서에 생성 시각·리비전·질의를 적는다. 유저 에스컬레이션(11.1절 커버리지 임계)의 근거가 이 보고서다. V&V KB의 지표 일곱(8.26절): 목표 파생률 · 기준 바인딩률 · V&V 전방 추적 커버리지 · 실행 통과율 · 변이 검출률 · 재현 실패율 · 검증 대응물 지연.
