...
이 환원은 계층의 특정 단계에 갇히지 않는다. 수직 링크(`refines`)의 후보는 주로 logical에 열거되지만, 수평 링크(`satisfies`·`constrains`·`verifies`)의 후보는 확정된 concrete 항목 사이에도 생긴다 — 확정된 시그니처가 어느 확정된 결정을 충족하는지 아직 모를 수 있다. **연결이 있는 곳이면 어디든 가능성이 표현된다.**
