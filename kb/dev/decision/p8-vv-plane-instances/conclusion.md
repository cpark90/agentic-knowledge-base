...
---
**결론** — V&V KB는 코어의 **두 번째 인스턴스**다. 일곱 plane의 판정 방식은 같고 실체만 다르다. **새 plane을 만들지 않는다.**

- `requirement` → **검증 목표** — 무엇이 보여져야 하는가. 요구에서 `derives-from`. 합의로 판정
- `decision` → **시나리오** — 어떤 자극으로, 왜. 논증 + ODD 정합
- `contract` → **합격 기준** — 판정식. 검증기와 대상 사이의 계약. 형식 검사
- `schema` → **케이스 데이터 형식** — 자극·기대값의 구조. 스키마 검사
- `artifact` → **검증기** — 케이스를 실행해 기준을 적용하는 산출물. 실행
- `annotation` → **판정 주석** — 결함 분류, 이탈 기록. 해소로 판정
