...
- **연동의 이유는 완주다.** 개발 계층의 각 높이에 V&V 계층의 같은 높이가 대응하고, 둘 사이의 검증 대응물(`verifies`)가 없으면 개발 계층이 다음 높이로 내려갈 수 없다 (7.3절). 검증이 개발 뒤의 단계가 아니라 같은 시점의 산출물이 된다.
- 코어를 새로 만들지 않는 이유는 두 벌의 규칙이 생기면 어느 쪽 규칙으로 판정하는지가 매번 문제가 되기 때문이다. 같은 청크·plane·level·링크를 쓰고 실체만 바꾼다 (7.2절).
- 되먹임은 링크가 아니라 일반화로 간다 — 실행 기록과 결함이 요구·ODD를 고치는 경로다 (7.4절·7.5절).
