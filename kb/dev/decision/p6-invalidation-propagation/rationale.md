...
- 4·5단계에서 구성체와 링크를 `invalidated`가 아니라 `suspect`로 두는 이유 — 부분의 무효가 전체의 무효를 함의하지 않는다. 판정은 재판정 경계에서 규칙 또는 사람이 한다 (r-012).
- 7단계 통지가 필요한 이유는 무효화가 조용히 일어나면 아무도 재검토하지 않기 때문이다. 범위를 수로 알려야 우선순위가 정해진다.
- 8단계가 이 절차를 상승과 잇는다 — 자주 깨지는 가정이 일반화 대상이다 (r-004).
