...
type: decision
level: concrete
title_ko: 제약 전파는 게이트와 재판정 경계에서만 실행한다
...
composite: {id: https://agentic-knowledge-base.dev/id/composite/560ccdfa-94a3-4a43-bc41-3697c44b69cc, title_ko: 제약 전파는 게이트와 재판정 경계에서만 실행한다, title: Run constraint propagation only at gates and rejudgement boundaries}
---
**결론** — 제약 전파는 **검사 게이트 통과 시점**과 **재판정 경계**(Part XI 입력의 재판정 경계)에서 실행한다. **편집마다 실행하지 않는다.**
