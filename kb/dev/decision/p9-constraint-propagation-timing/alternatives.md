...
---
**대안** `[안]` — 후보 링크의 도메인 축소는 **호 일관성(arc consistency)** 검사이고, 새 제약 도입 시 영향받는 변수만 다시 검사하는 증분 방식이 이미 성숙해 있다. **자체 구현하지 않고 기성 알고리즘을 쓴다** — 아직 미확정 제안이며, 채택 시에도 이 결정의 실행 시점 규율은 그대로 적용된다.
