...
- **검증(verification)** — 산출물이 요구를 충족하는가. 증거는 검증기 실행 결과(기준 대비 통과/실패). 환경 계층 1~5단계. 결함은 산출물·결정 수정으로 간다. 시나리오 출처는 `origin:designed`(요구에서 파생)
- **확인(validation)** — **요구 자체가 맞는가.** 증거는 실환경 관측·유저 피드백·ODD 이탈 빈도·사후분석. 환경 계층 5~6단계. 결함은 **요구·ODD 수정**(일반화)으로 간다. 시나리오 출처는 `origin:observed`(관측에서 일반화)

**확인의 결과가 개발 KB의 `requirement` plane으로 돌아가는 것이 요구가 바뀌는 유일한 정규 경로다.**

에이전트 검증(7.8절)은 확인의 특수 형태다 — 만드는 주체가 맞게 만들고 있는가를 묻고, 자극은 작업 집합이며 기준은 산출물 품질과 인지 누락률이다.
