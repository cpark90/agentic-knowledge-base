...
| 코드 파일, 문서 | 빌드, 사람 | tangle·weave 투영 (4.6절) |
| 결정 기록 (ADR) | 사람 | `decision` 구성체의 투영 |
| 변경 영향 보고 | 유저 | 12.6절 |

지표 — 하강 완주율(100%) · 상향 귀속률(100%) · 결정 완결률(concrete 청크가 있는 결정 비율 = 진행도) · `-space` 체류 시간(유저 피드백 병목) · 계약 선행률(100%) · 대안 기록률(100%).
