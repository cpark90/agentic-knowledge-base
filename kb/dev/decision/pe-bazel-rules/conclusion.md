...
---
**결론** — Bazel 규칙 (노트 부록 E.6): plane별 `*_chunk` 규칙 — provider(plane·level·해시·라벨), 링크 타입별 속성이 deps, TIM·거주표·동질성은 분석 시점 검사(위반 = 빌드 실패) / `composite` — 7±2·동질성 / `odd`·`scope`·`ontology` — 속성 provider, visibility 그룹, 3계층 검사 / `space` + `space_check_test` — 후보·제약·선호, 호 일관성, ODD 경계, 확정 제안, **deps 아님** / `kg`·`tangle`·`weave`·`trace_matrix`·`vv_report` — 투영, 출력은 소스 밖 / 게이트 테스트 7종, `bazel test //...` = 재판정 경계.
