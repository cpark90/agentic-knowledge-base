...
결정 청크(`chunks/decision/`)와 `docs/`다.
본문의 `(노트 N.N절)` 인용은 분해 전 설계 노트에 대한 출처 기록이다 —
규약은 `docs/README.md`.
...
채널 파일은 지식 산출물이 아니라 소통 기록이다 — **그래프 밖**이며 게이트 검사
대상이 아니다 (§6의 filegroup 규칙에서 제외되는 유일한 문서군). 규약 원본은
`docs/feedback/README.md`, 여기는 작성 스타일만 적는다.

- **[지킴]** 쓰기 경계: hci는 채널 전체, 다른 에이전트는 자기 항목(`agents/` lane,
  조사 lane의 담당 답)만. 이외 채널 파일은 조회 전용.
- **[지킴]** 한 파일에 한 주제 — 항목이 반영·승인의 단위다.
- **[지킴]** frontmatter의 `status` 어휘는 lane마다 다르다 — 섞지 않는다:
  유저 lane `open→approved`(approved는 유저만), 에이전트 lane
  `open→relayed→answered→closed`, 조사 lane `open→answered→closed`.
- **[지킴]** 에이전트가 쓰는 항목은 `.wip.md`로 작성하고 완료 시 rename한다.
  `*.wip.md`와 placeholder가 남은 항목은 처리하지 않는다.
- **[지킴]** 항목에서 지식을 가리킬 때는 이름이 아니라 **IRI·조건 id·`file:line`**으로
  가리킨다 — 채널은 그래프 밖이므로 링크가 유일한 연결이다.
- **[권장]** 형식·길이 제약 없음(유저 서술은 특히 자유). 승격될 내용이 보이면 hci가
  구체화 단계에서 청크 형식(§4)에 가깝게 정리해 두면 반영이 빠르다.
