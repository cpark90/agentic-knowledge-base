...
저작 스타일은 [`STYLEGUIDE.md`](STYLEGUIDE.md)가 원본이다.
...
3. **ODD가 상위다.** 스코프·가정은 `kb/odd/project-odd.yml`의 조건만 참조할 수 있다. 이 문서는
   OpenODD이고 TTL은 생성물이다. 없는 조건이 필요하면 ODD를 먼저 확장한다. 판정 방법과 등급이
   필수다.
4. **한 청크는 한 파일이다.** 지식도, 온톨로지도, shape도 파일 하나가 단위 하나다. 본문은
   42줄 이하다. 청크는 포맷이 아니라 **구조 규칙**이다.
5. **지식의 종류는 고유 용어로 부른다.** 고유 용어는 조건·개념·변수·후보·결정·가정·
   시그니처·함수·주석·관측이다. "X 청크"라 하지 않는다. 온톨로지 클래스 이름을 인용할 때는
   그대로 쓴다. `agt:DecisionChunk`가 그 예다.
...
8. **유저와의 상세 소통은 hci만 한다.** 다른 에이전트는 유저 피드백이 필요하거나 문제·
   ...
   않는다. 채널 규약 원본은 [`docs/feedback/README.md`](docs/feedback/README.md)다.
...
(`.claude/agents/hci.md`). orchestrator는 메인이라 이 표가 정의다. developer·vnv는
...
- dispatch 대상에게는 전체 컨텍스트가 아니라 **작업 집합**만 전달한다. 작업 집합은 스코프 ×
  level 창으로 거른 청크 집합이다([`docs/method.md` §8](docs/method.md#8-조회)). 저장소를
  통째로 컨텍스트에 싣지 않는다.
- **채널 쓰기 경계.** hci의 작성·수정 범위는 소통 채널(`docs/feedback/**`)과 자기 역할
  메모리(`.claude/agent-memory/hci/**`)뿐이다. hci의 조회 범위는 저장소 전체다. 다른
  에이전트는 채널에서 **자기 항목**만 작성·수정할 수 있다. 자기 항목은 `agents/` lane의
  항목과 조사 lane의 자기 담당 답이다. 이외 채널 파일은 조회만 가능하다. 승인된 항목은 hci가
  `handoff/` lane에 verdict·파급효과·반영 계획으로 넘긴다. 담당 역할의 인수 기록은 `agents/`
...
**유저 피드백 처리.** 파이프라인 원본은 [`docs/feedback/README.md`](docs/feedback/README.md)다.
1. 유저 항목 또는 에이전트 항목이 채널에 들어온다. 유저 항목은 조사 요청·구체화·제안이다.
2. hci가 검토·구체화한다. 조사가 필요하면 조사 lane으로 위임한다. 유저 판단이 필요한
   에이전트 항목은 유저 lane으로 중계한다.
3. 유저가 승인한 항목만 반영 계획대로 담당 write plane의 역할이 반영한다. 승인은
   `status: approved` 태깅 또는 담당 역할에게 준 구두 답이다. hci는 어느 쪽이든 수행하지
   않는다. 이를 `//docs/feedback:channel_lint_test`·writer 검사가 강제한다. 반영 후
   `bazel test //...` PASS를 확인하고 `agents/` 항목(`ref: handoff/…`)에 반영 결과를 기록한다.
4. hci가 반영 확인된 항목을 refresh한다.

## 소통 규칙 (문서 우선)

- **유저와의 상세 소통은 hci와 채널을 경유한다**(황금률 8). 다른 에이전트가 유저에게
  필요로 하는 것은 `agents/` lane에 항목으로 남긴다. hci가 확인해 유저 피드백을 받아온다.
- 상태·결정·주석은 해당 산출물에 쓴다. 결정은 `decision`에, 주석은 `annotation`에 쓴다.
  세션에는 무엇을 어디에 썼는지 요점만 남긴다. 임시 파일은 스크래치패드에 둔다.
- 채널 파일은 그래프 밖이다. 게이트 검사 대상이 아니다. 소통의 결론은 채널에 남기지 않고
  온톨로지·ODD·청크의 지식으로 승격한다.
- 유저의 판단이 필요한 항목은 **다섯 절**로 쓴다. 다섯 절은 질문 / 이미 정해진 것 / 현재
  상태 / 답이 가르는 것 / 선택지다. 한 줄 질문으로는 답을 받을 수 없다(유저 지적 2026-09-02).

## 언어 정책

산문은 한글, 식별자는 영어 소문자 케밥, 개념은 PascalCase(`agt:Assumption`), 라벨은 한/영
1:1이다. **지어낸 용어를 쓰지 않는다.** 확립된 표준어가 있으면 그것을 쓴다.
**산문은 단정 서술형이다**(유저 결정 2026-09-13). 문서·청크·채널 항목·세션 보고·dispatch 브리핑 전부
평서형 "…다"로 쓴다. 경어체·감탄·구어·추측 표현을 쓰지 않는다. 판단은 근거와 함께 단정하고, 근거가
없으면 적지 않는다(`STYLEGUIDE.md` §0). 경어·감탄은 게이트가 거부한다.
