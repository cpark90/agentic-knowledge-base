...
3. **ODD가 상위다.** 스코프·가정은 `odd/project-odd.ttl`의 조건만 참조할 수 있다. 없는
   조건이 필요하면 ODD를 먼저 확장한다 — 판정 방법과 등급이 필수다.
4. **한 청크는 한 파일이다.** 지식도, 온톨로지도, shape도 파일 하나가 단위 하나이며 본문
   42줄 이하다. 청크는 포맷이 아니라 **구조 규율**이다.
5. **지식의 종류는 고유 용어로 부른다** — 조건·개념·변수·후보·결정·가정·시그니처·함수·
   논평·관측. "X 청크"라 하지 않는다. 온톨로지 클래스 이름(`agt:DecisionChunk` 등)을 인용할
   때는 그대로 쓴다.
...
| **vnv** (dispatch) | 판정 전용: `bazel test //...` PASS 확인 + 결과 논평. **V&V KB(`kb/vv/`)의 유일한 편집 주체** — `verifies`의 주어는 V&V 청크뿐 | `annotation` + `kb/vv/` | `artifact`·`decision` | dispatch | ✗ |
...
작성, 청크 저작, 하강 전이, 후보 관리, 연결, 갱신, 조회, 투영, 상승, 검증, 영향 분석.
...
- 상태·결정·논평은 해당 산출물에 쓰고(결정은 `decision`, 논평은 `annotation`), 세션에는
